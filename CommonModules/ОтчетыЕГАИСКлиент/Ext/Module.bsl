...
Процедура ЗапроситьОтсутствующиеТТН(ФормаОтчета) Экспорт
	...
	Если ДанныеЗапросов.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Все ТТН ЕГАИС загружены, операция не требуется'"),, "ЗапроситьОтсутствующиеТТН");
		Возврат;
	КонецЕсли;
...
Процедура ЗапроситьВыделенныеДанные(ФормаОтчета) Экспорт
	...
	Если ДанныеЗапросов.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Операция недоступна для выделенных объектов'"),, "ЗапроситьВыделенныеДанные");
		Возврат;
	КонецЕсли;
