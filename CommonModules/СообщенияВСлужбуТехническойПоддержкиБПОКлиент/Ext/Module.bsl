...
Процедура ВыполнитьОтправкуСообщенияОбОшибке_ОтправкаСообщения(Результат, Контекст) Экспорт
	...
	Если ОбщегоНазначенияБПОКлиент.РазделениеВключено() 
		И ОбщегоНазначенияБПОКлиент.ПодсистемаСуществует("ТехнологияСервиса.ИнформационныйЦентр") 
		И ОбщегоНазначенияБПОСлужебныйВызовСервера.БТССодержитВозможностьОтравкиСообщений() Тогда
		
		// Вызов БТС
		МодульИнформационныйЦентрКлиент = ОбщегоНазначенияБПОКлиент.ОбщийМодуль("ИнформационныйЦентрКлиент");
		ПараметрыОбращенияВСлужбуПоддержки = МодульИнформационныйЦентрКлиент.НовыеПараметрыОбращенияВСлужбуПоддержки();
		ПараметрыОбращенияВСлужбуПоддержки.Тема = Контекст.ДанныеСообщения.Тема;
		ПараметрыОбращенияВСлужбуПоддержки.Текст = Контекст.ДанныеСообщения.Сообщение;
		ПараметрыОбращенияВСлужбуПоддержки.ИспользоватьСтандартныйШаблон = Ложь;
		ПараметрыОбращенияВСлужбуПоддержки.ДополнительныеДанные = Контекст.ДополнительныеДанные;
		ОбщегоНазначенияБПОСлужебныйВызовСервера.ПоместитьДанныеВоВременноеХранилище(Контекст.ДанныеВложений);
		
		ВложенияИнфоЦентра = Новый Массив();
		Для каждого Вложение Из Контекст.ДанныеВложений Цикл
			ВложениеИнфоЦентра = Новый Структура();
			ВложениеИнфоЦентра.Вставить("ИмяФайла", Вложение.Представление);
			ВложениеИнфоЦентра.Вставить("АдресВоВременномХранилище", Вложение.Данные);
			ВложенияИнфоЦентра.Добавить(ВложениеИнфоЦентра);
		КонецЦикла;
		ПараметрыОбращенияВСлужбуПоддержки.Вложения = ВложенияИнфоЦентра;
		
		МодульИнформационныйЦентрКлиент.ОткрытьФормуНовогоОбращенияВСлужбуПоддержки(ПараметрыОбращенияВСлужбуПоддержки);
		// Конец Вызов БТС
